...
import { resolveReplayTextMetrics, type ReplayTextMetrics, type VideoTextSizePreset } from './text-size';

const execFile = promisify(execFileCallback);

export type ReplayProvider = 'codex' | 'claude' | 'opencode' | 'gemini';
...
function resolveGeminiSessionsRoot(): string {
  return join(process.env.HOME || '', '.gemini', 'tmp');
}
...
function buildReplayTitle(provider: ReplayProvider, label: string): string {
  const providerTitle = provider === 'opencode' ? 'OpenCode' : provider[0]!.toUpperCase() + provider.slice(1);
...
async function parseGenericEventsFromFile(
  ...
  if (trimmed.startsWith('[')) {
    ...
    if (Array.isArray(parsed)) {
      records.push(...parsed.filter((entry): entry is Record<string, unknown> => !!entry && typeof entry === 'object'));
    }
  } else if (trimmed.startsWith('{')) {
    const parsed = JSON.parse(trimmed) as Record<string, unknown>;
    if (Array.isArray(parsed.messages)) {
      records.push(...parsed.messages.filter((entry): entry is Record<string, unknown> => !!entry && typeof entry === 'object'));
    } else if (Array.isArray(parsed.events)) {
      records.push(...parsed.events.filter((entry): entry is Record<string, unknown> => !!entry && typeof entry === 'object'));
    } else {
      records.push(parsed);
    }
  } else {
  ...
  for (const parsed of records) {
    ...
    if (projectFilter && typeof parsed.projectHash === 'string' && !parsed.projectHash.includes(projectFilter)) {
      continue;
    }
...
async function findGeminiSessionPath(query: string, inputPath?: string): Promise<string> {
  if (inputPath) {
    return resolve(inputPath);
  }

  const sessionsRoot = resolveGeminiSessionsRoot();
  ...
  if (matches.length === 0) {
    throw new Error(`No Gemini session found for query: ${query}`);
  }
  ...
  const ranked = await Promise.all(
    matches.map(async (path) => ({
      ...
      score: path.includes('/chats/') ? 2 : path.endsWith('/logs.json') ? 1 : 0
    }))
  );
  ranked.sort((left, right) => right.score - left.score || right.modifiedMs - left.modifiedMs);
  return ranked[0]!.path;
}

async function findLatestGeminiSessionPath(): Promise<string> {
  const sessionsRoot = resolveGeminiSessionsRoot();
  const matches = (await runCommand('find', [sessionsRoot, '-type', 'f', '(', '-path', '*/chats/session-*.json', '-o', '-name', 'logs.json', ')']))
    .split('\n')
    .map((line) => line.trim())
    .filter((line) => line.endsWith('.json'));

  if (matches.length === 0) {
    throw new Error('No Gemini sessions were found under ~/.gemini/tmp');
  }
...
async function findGeminiSessionBySelector(selector: string): Promise<string | null> {
  const sessionsRoot = resolveGeminiSessionsRoot();
  ...
  const matches = (await runCommand('find', [sessionsRoot, '-type', 'f', '-name', `*${normalizedSelector}*.json`]))
    .split('\n')
    .map((line) => line.trim())
    .filter((line) => line.endsWith('.json'));
...
async function resolveReplayInputPath(provider: ReplayProvider, options: ReplayRenderOptions): Promise<{ path: string; warnings: string[] }> {
  ...
  if (provider === 'gemini') {
    return {
      path: await findLatestGeminiSessionPath(),
      warnings: []
    };
  }
...
export async function resolveReplaySelection(options: ReplaySelectionOptions): Promise<ReplaySelectionResult> {
  ...
  if (options.provider === 'gemini') {
    ...
    const selectedPath =
      !selector || selector === 'latest'
        ? await findLatestGeminiSessionPath()
        : (await findGeminiSessionBySelector(selector)) ?? (await findGeminiSessionPath(selector));
    const stem = basename(selectedPath, extname(selectedPath));
    const label = stem.startsWith('session-') ? stem.slice('session-'.length) : stem;
    return {
      inputPath: selectedPath,
      outputPath: join(cwd, `asdf-tube-gemini-${sanitizeOutputSegment(label)}.mp4`),
      title: buildReplayTitle('gemini', label),
      warnings: selector ? [] : ['No session selector provided. Defaulted to the latest Gemini session.']
    };
  }
...
function buildStages(
  ...
  const continuationChars = Math.max(52, Math.floor((contentWidth - 12) / (textMetrics.bodyFontSize * 0.66)));
  const maxLines = Math.max(10, Math.floor((contentBottom - contentTop) / textMetrics.lineHeight));
...
function lineYExpression(stage: ReplayStage, absoluteRow: number, contentTop: number, lineHeight: number): string {
...
async function renderStagesToVideo(
  ...
  const maxVisibleLines = Math.max(10, Math.floor((contentBottom - contentTop) / textMetrics.lineHeight));
  ...
  stages.forEach((stage) => {
    ...
    filters.push(drawText(`turn ${stage.block.turnIndex}/${events.length}`, `w-text_w-${outerX + 18}`, barTextY, textMetrics.barTextFontSize, '0x8fa6b6', stageEnable, font));
    ...
    stage.visibleLines.forEach((entry) => {
      const y = lineYExpression(stage, entry.absoluteRow, contentTop, textMetrics.lineHeight);
      const clock = entry.lineIndex === 0 ? formatClockFromMs(entry.block.event.timestampMs - visibleStartMs) : '';
      const size = entry.lineIndex === 0 ? textMetrics.leadFontSize : textMetrics.bodyFontSize;
      ...
      if (clock) {
        filters.push(drawTextBaseline(clock, contentX, y, textMetrics.timestampFontSize, '0x6e8092', stageEnable, font));
      }
    ...
    stage.block.lines.forEach((line, lineIndex) => {
      const y = lineYExpression(stage, stage.block.absoluteRowStart + lineIndex, contentTop, textMetrics.lineHeight);
      const clock = lineIndex === 0 ? formatClockFromMs(stage.block.event.timestampMs - visibleStartMs) : '';
      const size = lineIndex === 0 ? textMetrics.leadFontSize : textMetrics.bodyFontSize;
  ...
  filters.push(drawText(barTitle, outerX + 18, barTextY, textMetrics.barTextFontSize, '0x8fa6b6', 'gte(t\\,0)', font));
...
export async function renderReplayVideo(options: ReplayRenderOptions): Promise<ReplayRenderResult> {
  ...
  const textMetrics = resolveReplayTextMetrics(options.textSizePreset ?? 'standard');
  ...
  const payloadPath = `${outputPath}.replay.json`;
  ...
  const stages = buildStages(events, width, height, speedMultiplier, textMetrics).slice(-maxPages);
  ...
  await writeFile(payloadPath, JSON.stringify(replayPayload, null, 2), 'utf8');
